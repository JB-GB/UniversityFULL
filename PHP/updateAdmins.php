<?php
    ...
    $stuID = $_GET['id'];
    $mod = "SELECT * FROM administradores WHERE id_Emp = '$stuID'";
    $resultMod = $conn->query($mod);

    if($resultMod->num_rows > 0){
        while($row = $resultMod->fetch_assoc()){
            $name1Admin = $row['prnom_Emp'];
            $name2Admin = $row['segnom_Emp'];
            $sur1Admin = $row['prape_Emp'];
            $sur2Admin = $row['segape_Emp'];
            $fono = $row['tel_Emp'];
            $mailAdmin = $row['mail_Emp'];
            $passAdmin = $row['pass_Emp'];
        }
    }
    ...
    if(isset($_POST['updata'])){
        ...
        //Actualizar datos
        $modificar_alumnos  = "UPDATE administradores 
        SET prnom_Emp = '$name1Admin', segnom_Emp = '$name2Admin',
        prape_Emp = '$sur1Admin', segape_Emp = '$sur2Admin', 
        tel_Emp = '$fono', mail_Emp = '$mailAdmin', 
        pass_Emp = '$passAdmin' 
        WHERE id_Emp = '$stuID'";
