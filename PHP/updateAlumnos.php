<?php
    ...
    $stuID = $_GET['id'];
    $mod = "SELECT * FROM alumnado WHERE id_Alumno = '$stuID'";
    $resultMod = $conn->query($mod);

    if($resultMod->num_rows > 0){
        while($row = $resultMod->fetch_assoc()){
            $name1Admin = $row['prnom_Alumno'];
            $name2Admin = $row['segnom_Alumno'];
            $sur1Admin = $row['prape_Alumno'];
            $sur2Admin = $row['segape_Alumno'];
            $fono = $row['tel_Alumno'];
            $mailAdmin = $row['mail_Alumno'];
            $passAdmin = $row['pass_Alumno'];
        }
    }
    ...
    if(isset($_POST['updata'])){
        ...
        //Actualizar datos
        $modificar_alumnos  = "UPDATE alumnado 
        SET prnom_Alumno = '$name1Admin', segnom_Alumno = '$name2Admin',
        prape_Alumno = '$sur1Admin', segape_Alumno = '$sur2Admin', 
        tel_Alumno = '$fono', mail_Alumno = '$mailAdmin', 
        pass_Alumno = '$passAdmin' 
        WHERE id_Alumno = '$stuID'";
